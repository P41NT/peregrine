...
protected:
	...
	std::unique_ptr<SimpleBCP<ClauseStoreDefault>> propagator;
	...
	void SetUp() override {
		...
		propagator = std::make_unique<SimpleBCP<ClauseStoreDefault>>(*clause_storage);
	}
};
...
TEST_F(PropagateTest, PropagateMultipleUnitClauses) {
	clause_storage->add_clause(std::vector<Lit>({ Lit{1} }));
	clause_storage->add_clause(std::vector<Lit>({ Lit{-2} }));
	propagator->propagate(*assignment);
	EXPECT_EQ(assignment->getVar(Var{ 1 }), LBool::TRUE);
	EXPECT_EQ(assignment->getVar(Var{ 2 }), LBool::FALSE);
}

TEST_F(PropagateTest, PropagateUnitClauseWithOtherLiterals) {
	clause_storage->add_clause(std::vector<Lit>({ Lit{1}, Lit{2}, Lit{3} }));
	propagator->propagate(*assignment);
	EXPECT_EQ(assignment->getVar(Var{ 1 }), LBool::TRUE);
