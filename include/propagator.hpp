...
concept Propagator =
    ...
    requires(Prop prop, const Clause &clauses, Assignment &assignment) {
      { prop.propagate(assignment) } -> std::same_as<std::optional<size_t>>;
    };

template <ClauseStorage Clauses> class SimpleBCP {
private:
...
public:
  ...
  explicit SimpleBCP(Clauses &_clauses)
      : clauses(_clauses), watches(clauses.num_vars()) {
    for (size_t clause_idx :
         std::views::iota(size_t{0}, clauses.num_clauses())) {
